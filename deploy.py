...
def execute_task(conn, script_name, script_path, script_ext, arguments, sudo_password):
    ...
    if script_ext == ".sh":
        ...
        if NEED_ADMIN:
            print(f"{script_path} {script_name}")
            conn.put(script_path, script_name)
            conn.run("chmod +x " + script_name, warn=True)
            conn.run(PREAMBLE + CMD, warn=True)
            conn.run("rm -rf " + script_name, warn=True)
        else:
            conn.put(script_path, script_name)
            conn.run("chmod +x " + script_name, warn=True)
...
def execute_script(conn, host, scripts, task, arguments):
    ...
    for script_name, script in scripts.items():
        ...
        sleep(1)
    if LOGGING:
        conn.run(f"tar cvf ./{host}_{conn.host}_log.tar ./*.log 1>/dev/null")
        conn.get(f"{host}_{conn.host}_log.tar", f"{host}_{conn.host}_log.tar")
        conn.run(f"rm -rf ./*.log && rm -rf ./{host}_{conn.host}_log.tar")

class Threader:
    def __init__(self, config, scripts, task, arguments, shost, sshkey):
        self.workers = 25
            ...
            if SINGLE_HOST:
                if shost in config.keys():
                    ...
                    if not SSH_KEY:
                        ...
                        print(f"{bcolors.INFO}Connecting to {shost} {bcolors.ENDC}")
                        ssh_config = Config(overrides={'sudo': { 'user': 'root', 'password': password}, 'connect_kwargs': { 'key_filename': sshkey }}) if NEED_ADMIN else Config(overrides={'user': username, 'password': password, 'connect_kwargs': {'key_filename': sshkey }})
                        try:
                            with Connection(host=address, user=username,
                                            port=port, config=ssh_config) as c:
                                futures.append(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sshkey', type=argparse.FileType('r'), help='Your private keyfile, make sure to change users to root and execute without --root if using a sshkey file')
    ...
    Threader(config, bashscripts, args.task, args.arguments, args.host, args.sshkey)
