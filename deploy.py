...
class DispatchingFormatter(logging.Formatter):
    ...
    """
    def __init__(self, formatters: Dict[str, logging.Formatter], default_formatter: logging.Formatter):
        """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--version', 
        action='version', 
        version=f'Deploy {__version__} by {__author__}'
    )
    ...
    source_group.add_argument(
        '-j', '--json', 
        type=str, 
        help='JSON configuration file with host definitions'
    )
    source_group.add_argument(
        '-c', '--csv', 
        type=str, 
        help='CSV file with host information'
    )
    ...
    parser.add_argument(
        '-i', '--host', 
        type=str, 
        help='Target a specific host by name'
    )
    parser.add_argument(
        '-l', '--local', 
        type=str, 
    ...
    task_group.add_argument(
        '-k', '--command', 
        type=str, 
        help='Execute a shell command on target hosts'
    )
    task_group.add_argument(
        '-t', '--task', 
        type=str, 
        help='Execute a specific script by name'
    )
    ...
    parser.add_argument(
        '-a', '--arguments', 
        type=str, 
        help='Arguments to pass to command or script'
    )
    parser.add_argument(
        '-L', '--list', 
        action='store_true', 
    ...
    filter_group.add_argument(
        '-n', '--network', 
        type=str, 
        help='Filter hosts by network using CIDR notation'
    )
...
def _load_hosts_from_csv(args: argparse.Namespace, logger: logging.Logger) -> Dict[str, Host]:
    ...
    try:
        accepted_os = args.os.split(',') if args.os else DEFAULT_CONFIG['default_os_filter']
...
def load_scripts(args: argparse.Namespace, logger: logging.Logger) -> Dict[str, Any]:
    ...
    try:
        ...
        if not scripts_dir.exists():
            logger.warning(f"Scripts directory '{scripts_dir}' not found, using current directory")
            scripts_dir = Path(".")
...
def setup_tasks(
    ...
    if args.command:
        ...
        if args.task not in scripts:
            raise ConfigurationError(f"Script '{args.task}' not found in loaded scripts")
